...
function _renderNotify(type, title, msg, time, icon) {
    ...
    setTimeout(() => {
        $(`#notify-Id-${id}`).css({ animation: "notifyFadeOut 0.5s" });
        setTimeout(() => {
            $(`#notify-Id-${id}`).remove();
            activeNotifyCount--;
            _processNotifyQueue();
...
document.addEventListener('mousemove', function (e) {
    ...
    const newLeft = Math.max(0, Math.min(window.innerWidth  - el.offsetWidth,  e.clientX - dragOffsetX));
    const newTop  = Math.max(0, Math.min(window.innerHeight - el.offsetHeight, e.clientY - dragOffsetY));
...
function applyAllPositions(positions) {
    ...
    // Atualiza animação após restaurar posições guardadas
    updateNotifyAnimation();
}
...
function loadhtml() {
    const css = `
        ...
        *{margin:0;line-height:0;user-select:none}
        ...
        .notify-container{position:relative;width:auto;margin-bottom:.63vw;bottom:0;height:auto;left:0}
        ...
        /* ── Animações adaptativas de notify ──
           Por defeito (lado esquerdo): entra e sai pela esquerda.
           Com classe notify-from-right (lado direito): entra e sai pela direita. */
        .notify-container{animation:notifyFadeIn .5s}
        .notify-from-right .notify-container{animation:notifyFadeInRight .5s}
        ...
        /* ── edit border ── */
        .ui-edit-active{outline:.13vw dashed rgba(49,181,255,.85) !important;outline-offset:.6vw !important;border-radius:.42vw !important}

        /* ── UI Edit Panel ── */
        #ui-edit-panel{position:absolute;top:50%;right:1.25vw;transform:translateY(-50%);width:13.5vw;background:rgba(4,12,30,.96);border:.06vw solid rgba(49,181,255,.25);border-radius:.73vw;box-shadow:0 0 2.5vw rgba(0,0,0,.6),0 0 1vw rgba(49,181,255,.08);z-index:9999;overflow:hidden;pointer-events:all}
        .uep-header{display:flex;align-items:center;gap:.63vw;padding:.83vw .9vw .73vw}
        .uep-header-icon{font-size:.83vw;color:rgba(49,181,255,.9);line-height:1 !important;flex-shrink:0}
        .uep-header-text{display:flex;flex-direction:column;gap:.18vw}
        .uep-title{color:#fff;font-family:"Gilroy-Bold";font-size:.67vw;font-weight:700;letter-spacing:.09vw;line-height:1.3 !important;text-transform:uppercase}
        .uep-subtitle{color:rgba(255,255,255,.4);font-family:"Gilroy-Medium";font-size:.5vw;line-height:1.3 !important}
        .uep-divider{height:.05vw;background:rgba(255,255,255,.07);margin:0}
        .uep-list{padding:.42vw 0}
        .uep-item{display:flex;align-items:center;gap:.63vw;padding:.6vw .9vw;cursor:pointer;transition:background .12s;position:relative}
        .uep-item:hover{background:rgba(49,181,255,.07)}
        .uep-item:hover .uep-item-indicator{background:rgba(49,181,255,.5)}
        .uep-item:hover .uep-item-arrow{opacity:.7;right:.73vw}
        .uep-item-active{background:rgba(49,181,255,.12) !important}
        .uep-item-active .uep-item-indicator{background:rgba(49,181,255,1) !important;box-shadow:0 0 .42vw rgba(49,181,255,.7) !important}
        .uep-item-active .uep-item-label{color:#fff !important}
        .uep-item-active .uep-item-arrow{opacity:1 !important;color:rgba(49,181,255,1) !important;right:.73vw !important}
        .uep-item-indicator{width:.35vw;height:.35vw;border-radius:50%;background:rgba(255,255,255,.2);flex-shrink:0;transition:background .12s,box-shadow .12s;line-height:1 !important}
        .uep-item-label{color:rgba(255,255,255,.65);font-family:"Gilroy-SemiBold";font-size:.6vw;font-weight:600;letter-spacing:.07vw;line-height:1.3 !important;text-transform:uppercase;flex:1;transition:color .12s}
        .uep-item-arrow{color:rgba(255,255,255,.2);font-size:.73vw;line-height:1 !important;position:absolute;right:.83vw;transition:opacity .12s,right .12s,color .12s}
        .uep-hint{display:flex;align-items:center;justify-content:center;gap:.42vw;padding:.42vw .9vw}
        .uep-hint-key{background:rgba(255,255,255,.08);border:.05vw solid rgba(255,255,255,.15);border-radius:.21vw;color:rgba(255,255,255,.5);font-family:"Gilroy-SemiBold";font-size:.46vw;padding:.15vw .35vw;line-height:1.4 !important;letter-spacing:.04vw}
        .uep-hint-text{color:rgba(255,255,255,.3);font-family:"Gilroy-Medium";font-size:.48vw;line-height:1.3 !important}
        .uep-footer{border-top:.05vw solid rgba(255,255,255,.07);padding:.63vw;display:flex;flex-direction:column;gap:.35vw}
        .uep-save{background:rgba(49,181,255,.18);border:.06vw solid rgba(49,181,255,.4);color:rgba(49,181,255,1);font-family:"Gilroy-SemiBold";font-size:.56vw;font-weight:600;letter-spacing:.06vw;text-transform:uppercase;text-align:center;padding:.52vw;border-radius:.35vw;cursor:pointer;line-height:1.4 !important;transition:background .12s,border-color .12s}
        .uep-save:hover{background:rgba(49,181,255,.28);border-color:rgba(49,181,255,.7)}
        .uep-cancel{color:rgba(255,255,255,.3);font-family:"Gilroy-Medium";font-size:.5vw;text-transform:uppercase;text-align:center;padding:.42vw;border-radius:.31vw;cursor:pointer;line-height:1.4 !important;letter-spacing:.04vw;transition:color .12s,background .12s}
        .uep-cancel:hover{color:rgba(255,255,255,.6);background:rgba(255,255,255,.04)}

        /* ── Animações ── */

        /* Notify: lado esquerdo (padrão) */
        @keyframes notifyFadeIn{0%{left:-20.83vw;opacity:0}100%{left:0;opacity:1}}
        @keyframes notifyFadeOut{0%{left:0;opacity:1}100%{left:-20.83vw;opacity:0}}

        /* Notify: lado direito (quando container está na metade direita) */
        @keyframes notifyFadeInRight{0%{right:-20.83vw;opacity:0}100%{right:0;opacity:1}}
        @keyframes notifyFadeOutRight{0%{right:0;opacity:1}100%{right:-20.83vw;opacity:0}}
